    ...
    def post(self, request):
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    user_file = user.postfile_set.create(file=request.FILES['file'], name=name,)
                    form = PostForm()
                    messages.info(request, "Thanks for upload. You are uploaded file.")

                    return HttpResponse(render(request, 'home.html', {'form': form, 'all_upload_files': all_upload_files}))
            else:

                form = PostForm()

            return render(request, 'home.html', {'form': form, 'all_upload_files': all_upload_files})
...
def add_comment_to_post(request, pk):
    ...
    if request.method == 'POST':
        ...
        coment_db = file.comments_set.create(comment=request.POST['comment'], author=request.user.first_name)
        form = CommentForm()
        context = {'file': file, 'form': form, 'all_comm_this_post': all_comm_this_post}
